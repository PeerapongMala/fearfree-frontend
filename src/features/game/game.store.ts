...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  fetchAnimalsByCategory: async (categoryId) => {
    ...
    try {
      ...
      logger.error("Error fetching animals", err);
  ...
  fetchAnimalAndStages: async (animalId) => {
    ...
    try {
      ...
      logger.error("Error fetching animal and stages", err);
  ...
  fetchGameRules: async () => {
    ...
    try {
      ...
      logger.error("Error fetching game rules", err);
  ...
  submitStage: async (stageId: number, isSuccess: boolean, note?: string) => {
    try {
      ...
      logger.error("Error submitting stage", err);
